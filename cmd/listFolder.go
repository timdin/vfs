...
func initListFolder(rootCmd *cobra.Command, storage storage.Storage) {
	...
	listFolder := &cobra.Command{
		...
		Short: "List folders",
		Long: `Usage: list-folders [username] [--sort-name|--sort-created] [asc|
		desc]`,
		Example: `list-folders [username] [--sort-name|--sort-created] [asc|
		desc]`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(res) == 0 {
				fmt.Printf("Warning: The user:[%s] doesn't have any folders\n", user)
			} else {
